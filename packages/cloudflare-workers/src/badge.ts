 ...
 */
export async function generateBadge(payload: BadgePayload, secret: string): Promise<string> {
...
export async function verifyBadge(
  ...
  try {
    ...
    return {
      method: payload.method as BadgeMethod,
      solveTimeMs: payload.solveTimeMs as number | undefined,
      verifiedAt: payload.verifiedAt as number,
    };
  } catch {
    return null;
  }
 ...
 */
export function generateShareText(badgeId: string, payload: BadgePayload, baseUrl: string): ShareFormats {
 ...
 */
export async function createBadgeResponse(
  method: BadgeMethod,
  secret: string,
  baseUrl: string,
  solveTimeMs?: number
 ...
 */
export function generateBadgeHtml(payload: BadgePayload, badgeId: string, baseUrl: string): string {
  const colors = METHOD_COLORS[payload.method];
  const label = METHOD_LABELS[payload.method];
  const icon = METHOD_ICONS[payload.method];
